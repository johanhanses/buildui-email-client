...
export default function Email() {
  ...
  function toggleMessage(mid: number) {
    if (selectedMessages.includes(mid)) setSelectedMessages((messages) => messages.filter((id) => id !== mid))
    if (!selectedMessages.includes(mid)) setSelectedMessages((messages) => [...messages, mid])
  }
                ...
                >
                  <div className="py-0.5">
                    <button
                      ...
                      className={`${
                        selectedMessages.includes(mid) ? 'bg-blue-500' : 'hover:bg-slate-200'
                      } block w-full cursor-pointer truncate rounded py-3 px-3 text-left`}
                    >
                      <p
                        className={`${
                          selectedMessages.includes(mid) ? 'text-white' : 'text-slate-500'
                        } font-mediu truncate text-sm`}
                      >
                        {titles[mid % titles.length][0]}
                      </p>
                      <p
                        className={`${
                          selectedMessages.includes(mid) ? 'text-blue-200' : 'text-slate-400'
                        } truncate text-xs`}
                      >
